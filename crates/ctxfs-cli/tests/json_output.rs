...
fn ctxfs() -> Command {
    ...
    cmd.env("CTXFS_SOCKET", "/tmp/ctxfs-test-nonexistent-socket.sock");
    cmd
}

#[test]
fn diag_json_emits_valid_json_object() {
    let output = ctxfs().args(["diag", "--json"]).output().expect("exec");
    ...
    assert!(parsed.get("bundle_id").is_some(), "missing 'bundle_id' key");
    assert!(parsed.get("backend").is_some(), "missing 'backend' key");
    assert!(
...
fn list_json_emits_valid_json_array_or_errors_cleanly() {
    let output = ctxfs().args(["list", "--json"]).output().expect("exec");
